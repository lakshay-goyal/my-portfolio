...
function NavBar() {
  ...
  const getDockStyle = (index) => {
    ...
    const scale = distance === 0 ? 1.42 : distance === 1 ? 1.2 : distance === 2 ? 1.08 : 1;
    const translateY = distance === 0 ? -10 : distance === 1 ? -5 : distance === 2 ? -2 : 0;
  ...
  return (
    <nav className="fixed inset-x-0 bottom-3 z-50 px-3 sm:bottom-5" aria-label="Primary navigation">
      <div
        ...
        onMouseLeave={() => setHoveredIndex(null)}
      >
        <div className="flex min-w-0 items-end gap-1">{dockItems.slice(0, navLinks.length).map(renderDockItem)}</div>
